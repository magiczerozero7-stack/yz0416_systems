...
import { 
  DIGITAL_KPI_DATA as KPI_DATA,
  DIGITAL_SECONDARY_KPI as SECONDARY_KPI,
  DIGITAL_PROJECT_TYPES as PROJECT_TYPES,
  DIGITAL_INVESTMENT_DIST as INVESTMENT_DIST,
  DIGITAL_MONTHLY_INVESTMENT as MONTHLY_INVESTMENT,
  DIGITAL_RISK_WARNINGS as RISK_WARNINGS,
  DIGITAL_QUARTERLY_DATA as QUARTERLY_DATA,
  DIGITAL_GROUP_DISTRIBUTION as GROUP_DISTRIBUTION
} from '../data';
...
const DigitalProjectView: React.FC<{ quarter: string, setQuarter: (q: string) => void, groupTab: string, setGroupTab: (tab: string) => void }> = ({
  quarter,
  setQuarter,
  groupTab,
  setGroupTab
  ...
  return (
    <div className="px-4 md:px-8 relative z-20 space-y-4 md:space-y-6 max-w-7xl mx-auto w-full">
      ...
      {/* 2. Secondary KPIs (Image 2) */}
      <div className="grid grid-cols-3 gap-3 md:gap-6">
        {SECONDARY_KPI.map((kpi) => (
          <div key={kpi.label} className="bg-white py-1.5 px-2 rounded-2xl shadow-sm border border-white flex flex-col items-center justify-center text-center">
            <span className="text-[#9CA3AF] text-xs md:text-sm font-normal mb-0">{kpi.label}</span>
            <span className="text-[#374151] text-lg font-normal leading-tight">{kpi.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <Card title="月度投资分析" className="md:col-span-2 lg:col-span-1">
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <Card title="项目风险预警" className="md:col-span-1">
          <div className="space-y-2 mt-2">
            {RISK_WARNINGS.map((risk) => (
              <div key={risk.id} className="flex items-center justify-between p-3 bg-[#F8FAFC] rounded-xl group">
                <div className="flex items-center space-x-3 min-w-0">
                  <FileText size={18} className="text-blue-500 shrink-0" />
                  <p className="text-sm font-bold text-[#374151] truncate">{risk.name}</p>
                </div>
                <span className="text-xs font-bold whitespace-nowrap ml-2" style={{ color: risk.color }}>
                  x {risk.tag}
                </span>
              </div>
            ))}
        ...
        <Card title="季度进度分析" className="md:col-span-1">
...
export default DigitalProjectView;
